...
class TestTagCategories:
    ...
    def test_new_embodiment_is_finetune_only(self):
        ...
        assert EmbodimentTag.NZ100 in FINETUNE_ONLY_TAGS
...
class TestEmbodimentTagConsistency:
    ...
    def test_nz100_modality_config(self):
        config = MODALITY_CONFIGS[EmbodimentTag.NZ100.value]
        assert config["video"].modality_keys == ["top", "wrist_left"]
        assert config["state"].modality_keys == [
            "left_arm",
            "left_gripper",
            "right_arm",
        ...
        assert config["state"].sin_cos_embedding_keys == ["left_arm", "right_arm"]
        assert config["action"].modality_keys == [
            "left_arm",
            "left_gripper",
            "right_arm",
            "right_gripper",
